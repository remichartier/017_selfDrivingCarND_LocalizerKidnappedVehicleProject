...
/**
 * History Modifications Remi Chartier - non exhaustive
 * v00 : initial file
 ...
 *		handling of yaw_rate == 0 to avoid divide by 0
 *		PASSED
 * v08 : cleanup before submit to Udacity
 */
...
#define DEBUG_01 false // debug prob / weight_sum
#define DEBUG_02 false // debug weights
#define DEBUG_03 false // debug weights & prob
#define DEBUG_04 false // debut init particle x,y,theta + apply Gaussian
 ...
 */
void ParticleFilter::applyGaussianNoise(double std[]) {
  /**
   * add normal Gaussiand Noise distribution to particles 
   * x,y,theta components
   */
   
  std::default_random_engine gen; // random engine to create a normal 
  								  // (Gaussian) distributions for x,y,theta
  ...
  for (int i = 0; i < num_particles; ++i) { 
    ...
    if(DEBUG_04){
      std::cout << "init() x,y,theta : " << particles[i].x << ", ";
      std::cout << particles[i].y << ", " << particles[i].theta << std::endl;
    }
  }    
}
...
void ParticleFilter::prediction(double deltat, double std_pos[], 
  ...
  if(yaw_rate == 0) {
    std::cout << "ParticleFilter::prediction() ERROR : yaw_rate=0 -->";
    std::cout << " dividing by 0 ==> we need to skip this update ..." << std::endl;
    return;
  }
...
void filterOutOfSensorRangeLandmarks(const Map &map_landmarks,
  ...
  for(unsigned int l=0; l < map_landmarks.landmark_list.size(); ++l){
    double xl(map_landmarks.landmark_list[l].x_f);
    double yl(map_landmarks.landmark_list[l].y_f);
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
  ...
  for(unsigned int i=0; i<(unsigned int)num_particles; ++i){  // For each particle in the particle filter
    ...
    multivariate_gaussian_probability(prob, obs_m, std_landmark,predicted);
  ...
  if(!status){
    ...
    std::cout << "ParticleFilter::updateWeights() : ERROR weight_sum = 0 ";
    std::cout << "results in division by 0" << std::endl;
...
void ParticleFilter::resample() {
  ...
  // picked randomly according to their previous weight
}
